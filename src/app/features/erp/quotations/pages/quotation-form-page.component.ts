...
import { apiErrorMessage, lineAmountPreview, parseNextDocNumber } from '../quotation.util';
...
@Component({
  ...
  styleUrl: './quotation-form-page.component.css',
})
export class QuotationFormPageComponent implements OnInit {
  ...
  readonly form = this.fb.group({
    ...
    quotationDate: this.fb.control<Date | null>(new Date(), Validators.required),
  ...
  ngOnInit(): void {
    ...
    if (idParam && idParam !== 'new' && !Number.isNaN(Number(idParam))) {
      ...
      this.quotationId.set(Number(idParam));
  ...
  private loadNextNumber(): void {
    this.api.getNextNumber().subscribe({
      next: (raw) => this.form.patchValue({ quotationNumber: parseNextDocNumber(raw) }),
      error: () => {},
    });
  }
    ...
    this.api.getById(id).subscribe({
      next: (q) => {
        this.form.patchValue({
          ...
          customerLedgerId: q.customerLedgerId,
          status: q.status,
        ...
        this.customerOptions.set([
          { label: `${q.customerName} (${q.customerCode})`, value: q.customerLedgerId },
        ]);
        this.lines.clear();
  ...
  submit(): void {
    ...
    if (this.isEdit() && this.quotationId() != null) {
      this.api
        .update(this.quotationId()!, {
          quotationDate: v.quotationDate ? new Date(v.quotationDate).toISOString() : undefined,
      ...
      const quotationNumber = String(this.form.getRawValue().quotationNumber ?? '').trim();
      if (!quotationNumber) {
        this.error.set('Quotation number is required. Refresh the page to load the next number.');
        this.saving.set(false);
        return;
      }
      this.api
