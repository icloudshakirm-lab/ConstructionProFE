  ...
  template: `
    <div class="mx-auto max-w-5xl space-y-4">
      <header class="space-y-1">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-50">Ledgers closing balance</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">GET /reports/ledgers/closing-balance?toDate=…</p>
      </header>
        ...
        <table class="cp-data-grid__table">
          ...
          <tbody>
            @if (loading()) {
              ...
              @for (r of rows(); track r.ledgerId) {
                <tr>
                  <td>{{ r.ledgerId }}</td>
                  <td>{{ r.code }}</td>
                  <td>{{ r.name }}</td>
                  <td>{{ r.closingBalance | number: '1.2-2' }}</td>
                </tr>
              }
            }
          </tbody>
