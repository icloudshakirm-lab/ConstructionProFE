...
export class PaymentVoucherComponent implements OnInit {
  ...
  loadNextVoucherNumber(type = 'PV'): void {
  ...
  readonly form = this.fb.group({
    ...
    headerLedgerId: this.fb.control<number | null>(null), // Credit ledger for Payment
  ...
  private buildTransactionRequestDraft(): CreateTransactionRequest {
    ...
    const children: CreateChildTransactionRequest[] = [
      ...
      ...lines.map(l => ({
        description: l.description || 'Payment line',
        ledgerId: l.ledgerId,
    ...
    return {
      ...
      description: payload.voucherNumber ? `Payment voucher ${payload.voucherNumber}` : 'Payment voucher',
  ...
  private buildTransactionRequest(): CreateTransactionRequest {
    ...
    if (!headerLedgerId) throw new Error('Select Payment Account (Bank/Cash) before saving.');
    ...
    const children: CreateChildTransactionRequest[] = [
      {
        description: `Payment total: ${payload.voucherNumber}`,
        ledgerId: headerLedgerId,
        ledgerEntryType: 'Credit',
        ledgerAmount: totalAmount,
      },
      ...lines.map(l => ({
        description: l.description || 'Payment entry',
        ledgerId: l.ledgerId,
        ledgerEntryType: 'Debit',
        ledgerAmount: l.amount
      }))
    ...
    return {
      ...
      type: 'Payment',
      date: this.buildVoucherDateIso(),
      description: `Payment voucher ${payload.voucherNumber}`,
      childTransactions: children,
    };
  }
