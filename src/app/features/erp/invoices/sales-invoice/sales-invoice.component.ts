...
import { generateSalesInvoiceNumber } from '../utils/invoice-number';
...
export class SalesInvoiceComponent implements OnInit {
  ...
  loadNextVoucherNumber(type = 'SI'): void {
      ...
      next: (tx: any) => {
        ...
        if (headerChild) {
          if (headerChild.ledgerName) {
            this.drLedgerSearch$.next(headerChild.ledgerName);
          }
        ...
        if (linesChildren.length > 0) {
          ...
          const crLedgerId = firstLine.ledgerId;
          
          if (firstLine.ledgerName) {
            this.crLedgerSearch$.next(firstLine.ledgerName);
          }
          this.form.patchValue({ crLedgerId });
          this.crLedgerOptions.update(opts => {
            if (!opts.find(o => o.value === crLedgerId)) {
              return [...opts, { label: firstLine.ledgerName || 'Unknown', value: crLedgerId }];
            }
            return opts;
          });
          ...
          linesChildren.forEach((line: any) => {
            const rowGroup = this.fb.group({
              ...
              lineKind: [line.inventoryType === 'In' ? 'return' : 'sale'],
  ...
  /**
   * Draft API payload for the JSON preview.
   * Must never throw — it updates on every input change, even when the form is incomplete.
   */
  private buildTransactionRequestDraft(): CreateTransactionRequest {
    ...
    const itemChildren: CreateChildTransactionRequest[] = lines.map((l) => {
      ...
      return {
        description: l.itemName || 'Invoice line',
    ...
    return {
      ...
      description: payload.invoiceNumber ? `Sales invoice ${payload.invoiceNumber}` : 'Sales invoice',
  ...
  private buildTransactionRequest(): CreateTransactionRequest {
    ...
    // Validate required swagger fields as best-effort before submit.
    if (!payload.invoiceNumber) {
      throw new Error('Missing invoice number.');
    }
    if (!drLedgerId || !crLedgerId) {
      throw new Error('Select Party A/C and Sales Account before saving.');
    }
    if (lines.length === 0) {
    ...
    const itemChildren: CreateChildTransactionRequest[] = lines.map((l) => {
      const qty = Number(l.qty) || 0;
      const rate = Number(l.rate) || 0;
      const amount = Math.round(qty * rate * 100) / 100; // Qty x Rate (rounded)
      const itemId = l.itemId ?? null;
      return {
        description: l.itemName || 'Invoice line',
        reference: l.batch || null,
        ...
        // Item record link (itemId = Item.Id)
        itemId: itemId != null ? String(itemId) : undefined,
        quantity: qty,
        rate: rate,
      };
    ...
    const children: CreateChildTransactionRequest[] = [
      // First entry: Party ledger + total amount (no item)
      {
        description: 'Sales invoice total',
        ledgerId: drLedgerId,
        ledgerEntryType: 'Debit',
      ...
      // Then each line as child transaction (Cr ledger)
      ...itemChildren,
    ];
