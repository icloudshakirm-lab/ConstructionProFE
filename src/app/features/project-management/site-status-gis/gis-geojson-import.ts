import { getProgressDefinition, parseProgressStatus } from './site-progress.model';
...
export function metaFromGeoJsonProperties(
  props: Record<string, unknown> | undefined,
  defaults: { strokeColor: string; strokeWeight: number }
): GisSketchMeta {
  const status = parseProgressStatus(props?.['progressStatus']);
  const def = status ? getProgressDefinition(status) : undefined;
  return {
    strokeColor:
      typeof props?.['strokeColor'] === 'string'
        ? (props['strokeColor'] as string)
        : (def?.color ?? defaults.strokeColor),
    strokeWeight:
      typeof props?.['strokeWeight'] === 'number'
        ? (props['strokeWeight'] as number)
        : (def?.weight ?? defaults.strokeWeight),
